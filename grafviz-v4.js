...
d3.json('rtu-data/' + wdKey + '-rtu-data.json')
.then((json) => {
		console.log('rtu ok')
		nodes = json.nodes
		params = json.params
  ...
  .catch((error) =>  {
    console.log('rtu data error',error,load_from_WD,wdjs)
		if (load_from_WD) {
...
function init() {
    ...
    var force = d3.forceSimulation(net.nodes)
...
crsrText.attr("display","none");
    ...
    let hull = hullg.selectAll("path.hull")
        ...
        .style("opacity", d => d.parentId == "root" ? 0.1 : d.parentId==focus? 1 : Math.min(0.05 , Math.max(0.1, 1 - (nodes[d.parentId].visibleDepth) / 3)))
        //.attr("d", drawCluster)
...
function drawCluster(d) {
    return d3.curveCardinalClosed()(d.path); // 0.8
}
