...
function adaptZoom() {
    ...
    vis.transition()
        .duration(zoomCounter<=1?2000:100)
        .call(zoom
            .scale(scaleFactor)
...
function buildNodesLinks(data) {
    ...
    const keys = data.parms?Object.keys(data.params):[]
    for (let k in keys) {
    ...
    for (let i in data.links) {
        ...
        if (linki.target != "") {
            ...
            if (!(linki.target in nodes)) {
                console.log(i, "target inconnu", linki,linki.parentId);
                let a = bb;
            }
            ...
            nodes[linki.source].linked.push(nodes[linki.target]);//used for focus
...
function init(adapt) {
    ...
    force = d3.layout.force()
        ...
        .linkStrength(function(l, i) {
            return (l.type == "Member of") ? .3 : 0.1;
            //((l.source.id === focus) || (l.target.id === focus) ||

        })
        .charge(function(n, i) { return (n.id === focus ? -200 * n.radius : -150 * n.radius); })
        //.gravity(0.1)
        .chargeDistance(800)
        .friction(.4)
        .start();
...
crsrText.attr("display","none");
    ...
    force.on("tick", function(e) {
        ...
        node
        .each(collide(e.alpha*.3))//<0.1? e.alpha/2 : 0))
        .each(function(d) {
    ...
    quadtree.visit(function(quad, x1, y1, x2, y2) {
      if (quad.point && (quad.point !== d) ) {
        var x = d.x - quad.point.x,
            y = d.y - quad.point.y,
            l = Math.sqrt(x * x + y * y),
            r = (d.radius + quad.point.radius +  padding+(d.parentId==quad.point.parentId?0:400))//*(quad.point.id==focus?2:1);
        if (l < r){
          l =- (l-r)/ l* alpha;
          d.x -= (x *= l);
          d.y -= (y *= l);
